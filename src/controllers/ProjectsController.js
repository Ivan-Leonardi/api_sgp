...
class ProjectsController {
    ...
    async index(req, res) {
        ...
        const projectsWithDevelopers = projects.map(project => {
            const projectDevelopers = userDevelopers.filter(dev => dev.project_id === project.id);
    ...
    async update(req, res) {
        ...
        try {
            ...
            if (currentStatus && currentStatus.status === 'iniciado') {
                ...
                return res.status(400).json({ error: 'O projeto não pode ser finalizado, pois não está no status "iniciado".' });
            }
        } catch (error) {
            console.error('Erro ao finalizar o projeto:', error);
            return res.status(500).json({ error: 'Erro interno ao finalizar o projeto.' });
        }
    }
}
